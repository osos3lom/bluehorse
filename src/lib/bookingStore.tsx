...
function useBookingStoreState() {
  ...
  const hasAdventures = state.cart.some((item) => item.category === 'adventures');
  const hasNightlife = state.cart.some((item) => item.category === 'nightlife');
  const hasCorporate = state.cart.some((item) => item.category === 'corporate');
  const hasEvents = state.cart.some((item) => item.category === 'events');
  const primaryCategory: ServiceCategory = state.cart[0]?.category || 'adventures';
